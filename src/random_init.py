...
from src.train_nn import ex, create_mlp_layers, save_weights, create_cnn_layers
...
ex.run(named_configs=['mlp_config', 'cnn_config'])
...
def compute_ncut_random_init_cnn():

    model = tf.keras.Sequential(create_cnn_layers())
    model.compile(loss='categorical_crossentropy',
                  optimizer='adam',
                  metrics=['accuracy'])
